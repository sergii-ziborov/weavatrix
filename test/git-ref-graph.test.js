...
test('Git-ref graph builds an immutable baseline without changing the worktree', async (t) => {
  ...
  writeFileSync(join(repo, 'test', 'value.test.js'), "import {value} from '../src/value.js'\n")
  ...
  const noTests = await buildGraphAtGitRef(repo, 'HEAD', {mode: 'no-tests'})
  assert.equal(noTests.ok, true, noTests.error)
  assert.equal(noTests.graph.graphBuildMode, 'no-tests')
  assert.ok(noTests.graph.nodes.some((node) => node.id === 'src/value.js'))
  assert.ok(!noTests.graph.nodes.some((node) => node.source_file === 'test/value.test.js'))
